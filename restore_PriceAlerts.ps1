...
$lastdiff=(ls Z:\FE\DBS-CLUSTER*PriceAlerts*DIFF* | Sort-Object -Descending {$_.LastWriteTime})[0]
$lastfull=(ls Z:\FE\DBS-CLUSTER*PriceAlerts*FULL* | Sort-Object -Descending {$_.LastWriteTime})[0]
if ($lastdiff.LastWriteTime -gt $lastfull.LastAccessTime){
 ...
 $s2="$dest\"+"PriceAlertsDiff.bak"
 Remove-Item $s2 -Force
 Rename-Item -Path $s1 -NewName $s2 -Force
 Invoke-Sqlcmd -InputFile C:\Users\r.jank\scripts\pricealerts_restore_fulldiff.sql -Username "r.jank" -Password  "MiF6Y-r_sc52" -QueryTimeout 65500 -OutputSqlErrors 1 -Verbose 4>&1 | Out-File C:\Users\r.jank\logs\pricealerts_restore_log.txt -Append
}
else {
$azcopycmd=[string]::Format("{0} /source:{1} /dest:{2} /sourcekey:{3} /y /Pattern:{4}",$azcopypath, $source, $dest, $mykey, $lastfull.Name);
 ...
 $s2="$dest\"+"pricealertsfull.bak"
 Remove-Item $s2 -Force
 Rename-Item -Path $s1 -NewName $s2 -Force
 Invoke-Sqlcmd -InputFile C:\Users\r.jank\scripts\pricealerts_restore_full.sql -Username "r.jank" -Password  "MiF6Y-r_sc52" -QueryTimeout 65500 -OutputSqlErrors 1 -Verbose 4>&1 | Out-File C:\Users\r.jank\logs\pricealerts_restore_log.txt -Append
